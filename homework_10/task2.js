...
let fighter = (fighterName) => {
  fighterName.getName = function () {
    return this.name;
  }
  fighterName.block = function () {
    return Math.random() >= 0.5;
  }
  fighterName.getStats = function () {
    let stats = {
      name:this.name,
      attack:this.attack,
      hp:this.hp
  ...
  fighterName.combatHistory = {
    wins:0,
    loses:0
  };
  fighterName.getCombatHistory = function () {
    let combatHistory = {
  ...
  fighterName.fight = function (enemy) {
    if (enemy.block === true) {
      return false;
    } else if (enemy.hp > fighterName.attack) {
      enemy.hp -= fighterName.attack;
      return true;
    } else if (fighterName.attack >= enemy.hp) {
      fighterName.combatHistory.wins += 1;
      enemy.combatHistory.loses += 1;
      return true;
    }
  }
  return fighterName;  
